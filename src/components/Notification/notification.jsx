...
function NotificationPage() {
  ...
  const onToggleCheck = (evt, label) => {
    if (evt.currentTarget.checked) {
      ...
      setCheckItems(prevItem => (prevItem?.filter(i => i !== label) ?? []))
    }
  }
  ...
  useEffect(() => {
    setCheckItems(typeof localStorage.getItem('notifications') === String ? JSON.parse(JSON.parse(localStorage.getItem('notifications'))) ?? [] : JSON.parse(localStorage.getItem('notifications')) ?? [])
  }, [])
  ...
  return (
    <div className="p-4">
      ...
      {['Incoming Orders', "Store performance", "Reviews and ratings", "Payment", "Delivery updates", "Customer's enquiry"]?.map((item, index) => (
        <CustomCheckBox label={item} key={item} onChangeFunc={onToggleCheck} checked={checkedItems?.includes(item)} />
      ))}
      
      
      <div role='button' onClick={submitForm} className='flex items-start md:items-center md:justify-center'>
...
const CustomCheckBox = ({label, checked, onChangeFunc}) => {
  return (
    ...
    <label className="inline-flex items-center">
      <input type="checkbox" checked={checked} onChange={(evt) => onChangeFunc(evt, label)} className="w-5 h-5 text-white bg-[#33CC9F] border-0 rounded checked:bg-[#33CC9F] checked:border-transparent form-checkbox" />
